...
const NavBar = ({ navBar2, showCase1Page }) => {
  ...
  const hideNav = () => setViewSideNav(false);
  ...
  // ✅ BODY SCROLL LOCK
  useEffect(() => {
    document.body.style.overflow = viewSideNav ? "hidden" : "auto";
  }, [viewSideNav]);
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* MOBILE ICON */}
          <FaBars
            onClick={() => setViewSideNav(true)}
            className="text-2xl cursor-pointer max-lg:block hidden"
          />
        </nav>
      </div>
      ...
      {/* SIDEBAR */}
      <div
        className={`fixed top-0 left-0 h-full w-72 bg-white z-[9999] transform transition-transform duration-300 ${
          viewSideNav ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex justify-between items-center p-5 border-b">
          <img className="w-32" src="/Homyz-logo2.png" alt="logo" />
          <button onClick={() => setViewSideNav(false)}>✕</button>
        </div>
